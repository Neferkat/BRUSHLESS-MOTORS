...
void loop() {
  ...
  x3=x2;
  x2=x1;
  x1=pulseIn(encoder,HIGH); //read Distance sensor SHARP 2Y0A21
  pos=(x5+x4+x3+x2+x1)/5;
  //pos=(x3+x2+x1)/3;
  //pos=(x1+x2)/2;  
...
*/
  ...
  //torque=constrain(torque,0,30);    //limit maximum torque 0-100 %, you can limit the maximum torque you want the motor to have i.e. instead of 100, use 60 for 60% of the maximum torque available, motor runs slower and cooler ;) 
...
void move()
{
  const int offset=8; //**ADJUST THIS VALUE** OFFSET needed for Syncronization, unique to each motor as it depends on manufacturing.
